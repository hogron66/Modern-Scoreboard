...
const COLOR_THEMES = [
  { key: "black", label: "Black", background: "#000000", foreground: "#f7f7f7", muted: "#777777" },
  { key: "blue", label: "Blue", background: "#123f8c", foreground: "#ffffff", muted: "#bdd1ff" },
  { key: "lightblue", label: "Light blue", background: "#87ceeb", foreground: "#000000", muted: "#17465a" },
  { key: "green", label: "Green", background: "#16753b", foreground: "#ffffff", muted: "#bce5c9" },
  { key: "red", label: "Red", background: "#a92323", foreground: "#ffffff", muted: "#ffc6c6" },
  { key: "orange", label: "Orange", background: "#ef8b24", foreground: "#000000", muted: "#4f2a00" },
  { key: "yellow", label: "Yellow", background: "#f2d43d", foreground: "#000000", muted: "#514600" },
  { key: "grey", label: "Grey", background: "#b8b8b8", foreground: "#000000", muted: "#3b3b3b" },
  { key: "white", label: "White", background: "#ffffff", foreground: "#000000", muted: "#555555" }
];
...
function loadState() {
  try {
    ...
    state = {
      ...
      crowdEnabled: false,
      backgroundColor: COLOR_THEMES.some((theme) => theme.key === saved.backgroundColor)
...
function render() {
  const theme = COLOR_THEMES.find((item) => item.key === state.backgroundColor) || COLOR_THEMES[0];
  document.documentElement.style.setProperty("--background", theme.background);
  document.documentElement.style.setProperty("--white", theme.foreground);
  document.documentElement.style.setProperty("--line", theme.foreground);
  document.documentElement.style.setProperty("--grey", theme.muted);
  document.documentElement.style.colorScheme = theme.foreground === "#000000" ? "light" : "dark";
  const themeColorMeta = document.querySelector('meta[name="theme-color"]');
  if (themeColorMeta) themeColorMeta.content = theme.background;
  ...
  elements.colorButton.textContent = "COLOR";
  elements.colorButton.setAttribute("aria-label", `Background color: ${theme.label}. Tap for next color.`);
...
function cycleBackgroundColor() {
  const currentIndex = COLOR_THEMES.findIndex((theme) => theme.key === state.backgroundColor);
  const nextIndex = (currentIndex + 1) % COLOR_THEMES.length;
  state.backgroundColor = COLOR_THEMES[nextIndex].key;
  saveState();
  render();
  showToast(`Background: ${COLOR_THEMES[nextIndex].label}`);
}
...
async function toggleFullscreen() {
  ...
  if (isAppleTouchDevice) {
    showToast(isInstalled
      ? "The app is already running full screen."
  ...
  try {
    if (document.fullscreenElement) {
      ...
      showToast("Install the app for a full-screen scoreboard.");
    }
  } catch (_error) {
...
function bindEvents() {
  ...
  elements.colorButton.addEventListener("click", cycleBackgroundColor);
